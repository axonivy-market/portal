...
public class IvyTaskTransformer {
  ...
  private boolean isNoConfigurationMode;

  public IvyTaskTransformer(String serverUrl){
    this.serverUrl = serverUrl;
    this.isNoConfigurationMode = true;
  }
  public IvyTaskTransformer(String serverUrl, boolean isNoConfigurationMode) {
    this.serverUrl = serverUrl;
    this.isNoConfigurationMode = isNoConfigurationMode;
  }
  ...
  public IvyTask transform(ITask t, boolean addCaseToTask) {
    ...
    if (t.getActivator() != null) {
      result.setActivatorName(t.getActivatorName().replaceFirst("#", ""));
      ISecurityMember activator = t.getActivator();
      if (activator != null) {
        String fullName = activator.getDisplayName();
        if (activator.isUser()) {
    ...
    if (t.getExpiryActivatorName() != null) {
      result.setExpireActivatorName(t.getExpiryActivatorName().replaceFirst("#", ""));
      ISecurityMember expiryActivator = t.getExpiryActivator();
      if (expiryActivator != null) {
        String fullName = expiryActivator.getDisplayName();
    ...
    if (t.getOriginalActivatorName() != null) {
      result.setOriginalActivatorName(t.getOriginalActivatorName().replaceFirst("#", ""));
      ISecurityMember originalActivator = t.getOriginalActivator();
      if (originalActivator != null) {
        String fullName = originalActivator.getDisplayName();
  ...
  private String getStartLink(ITask task) {
    ...
    String url = specifyServerUrl(serverUrl);
  ...
  private String specifyServerUrl(String serverUrl){
    String newURL = serverUrl;
    String externalHost = SystemProperties.getSystemPropertyValue(SystemProperties.EXTERNAL_HOST_NAME);
    if (!isNoConfigurationMode && !externalHost.isEmpty()) {
      try {
        URL url = new URL(serverUrl);
        newURL = new URL(url.getProtocol(), externalHost, url.getPort(), "").toString();
