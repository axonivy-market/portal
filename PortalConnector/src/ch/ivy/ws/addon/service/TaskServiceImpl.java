...
public class TaskServiceImpl extends AbstractService implements ITaskService {
      ...
      return securityManager().executeAsSystem(new Callable<TaskServiceResult>() {
        ...
        public TaskServiceResult call() throws Exception {
          ...
          if (taskId == null) {
            ...
            IvyTask ivyTask = new IvyTaskTransformer(serverUrl).transform(t);
            ...
            if (t != null) {
              try {
                ...
                if (workflowSession != null) {
                  ...
                  result = new IvyTaskTransformer(serverUrl).transform(t);
                }
              } catch (Exception e) {
                // Invalid username
                List<Object> userText = new ArrayList<Object>();
  ...
  @Override
  public TaskServiceResult resetTask(String currentUserName, final Integer taskId, String serverUrl) throws WSException {
    try {
      return securityManager().executeAsSystem(new Callable<TaskServiceResult>() {
        @Override
        public TaskServiceResult call() throws Exception {
    ...
    try {
      return securityManager().executeAsSystem(
          () -> {
            ...
            IvyTaskTransformer transformer = new IvyTaskTransformer(serverUrl, isNoConfigurationMode);
            tasks.forEach(task -> {
              IvyTask ivyTask = transformer.transform(task);
  ...
  private TaskQuery createTaskQuery(TaskSearchCriteria taskSearchCriteria) throws Exception {
    ...
    if (taskSearchCriteria.hasInvolvedUsername() && !taskSearchCriteria.isIgnoreInvolvedUser()) {
      ...
      AvailableAppsResult availableAppsResult = findAvailableApplicationsAndUsers(involvedApplications,
          involvedUsername);
      ...
      return securityManager().executeAsSystem(new Callable<TaskServiceResult>() {
        ...
        public TaskServiceResult call() throws Exception {
          ...
          if (workerUser != null) {
            ...
            String workerName = fullName == null || fullName.isEmpty() ? workerUser.getName() : workerUser
                .getFullName() + "(" + workerUser.getName() + ")";
  ...
  private TaskQuery queryForKeyword(String keyword) {
    ...
    TaskQuery filterByKeywordQuery = TaskQuery.create().where().or().name().isLike(containingKeyword).or()
        .description().isLike(containingKeyword).or().customVarCharField1().isLike(containingKeyword).or()
        .customVarCharField2().isLike(containingKeyword).or().customVarCharField3().isLike(containingKeyword).or()
        .customVarCharField4().isLike(containingKeyword).or().customVarCharField5().isLike(containingKeyword);
  ...
  private TaskQuery queryForExpiry(Date date) {
    ...
    TaskQuery priorityQuery = TaskQuery.create().where().expiryTimestamp().isGreaterOrEqualThan(date).and()
        .expiryTimestamp().isLowerThan(dateAfter1Day);
   ...
   * 
   * @param query
   *          given {@link TaskQuery}
   * @param startIndex
   *          starts from 0
   * @param count
   *          used -1 to find all from startIndex
   * @return
   */
