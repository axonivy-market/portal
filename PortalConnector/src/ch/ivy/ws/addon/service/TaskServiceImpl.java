          ...
          () -> {
            ...
            tasks.forEach(task -> {
              ...
              if (taskSearchCriteria.hasInvolvedUsername()) {
                ...
                ivyTask.setCanResume(canUserResumeTask);
                ivyTask.setCanChangePriority(hasPermissionToChangeOriginalPriority(involvedUsername, task.getApplication()));
  ...
  public void save(IvyTask task) throws WSException {
    try {
      securityManager().executeAsSystem(() -> {
        ...
        // As of now, expiryTimestamp and originalPriority are the only fields supported for updating.
