...
As0 f0 inParamDecl '<java.lang.Long caseId> param;' #txt
As0 f0 inParamTable 'out.caseId=param.caseId;
' #txt
...
As0 f0 callSignature showAdditionalCaseDetails(Long) #txt
...
As0 f6 startMethod start(ch.ivyteam.ivy.workflow.ICase) #txt
As0 f6 type internaltest.AdditionalCaseDetailsData #txt
As0 f6 requestActionDecl '<ch.ivyteam.ivy.workflow.ICase remoteCase> param;' #txt
As0 f6 requestMappingAction 'param.remoteCase=in.iCase;
' #txt
...
' #txt
As0 f212 actionCode ivy.wf.findCase(in.caseId); #txt
...
As0 f212 mainOut f3 tail #connect
