...
public class PortalConnectorDetector {
  ...
  private String getServerURL(IServerPortResolver serverPortResolver) throws UnknownHostException, MalformedURLException {
	IApplicationConfigurationManager appConfig = ServerFactory.getServer().getApplicationConfigurationManager();
    String serverAddress = getServerAddress();
    String serverProtocol;
    int serverPort;
    if(appConfig.getSystemProperty("WebServer.HTTP.Enabled").getBooleanValue()) {
      serverProtocol = IServerPortResolver.HTTP_PROTOCOL;
      serverPort = serverPortResolver.getServerPort(serverAddress, serverProtocol);
    } else if (appConfig.getSystemProperty("WebServer.HTTPS.Enabled").getBooleanValue()) {
  ...
  private String getServerAddress() {
    String serverAddress = ServerFactory.getServer().getApplicationConfigurationManager().getSystemProperty("WebServer.HTTP.Address").getValue();
