...
public class RemoteProcessStartMapper {
  ...
  protected static RemoteProcessStart mapProcessStart(IvyProcessStart t, Server server) {
    ...
    result.setFullRequestPathWithDocId(t.getFullRequestPathWithDocId());
    result.setStartLink(t.getStartLink());
    result.setServerAddress(t.getServerAdress());
   ...
   */
  public static List<RemoteProcessStart> mapProcessStarts(List<IvyProcessStart> ivyProcessStarts, Server server) {
    List<RemoteProcessStart> outStarts = List.create(RemoteProcessStart.class);

    for (IvyProcessStart t : ivyProcessStarts) {
      RemoteProcessStart start = mapProcessStart(t, server);
