[Ivy]
[>Created: Thu Mar 03 10:33:30 ICT 2016]
...
Ts0 @RichDialogMethodStart f67 '' #zField
Ts0 @RichDialogProcessEnd f68 '' #zField
Ts0 @GridStep f70 '' #zField
...
' #txt
Ts0 f26 actionCode 'in.tasks.remove(in.selectedTask);
in.expandedTask = null;' #txt
...
<elementInfo>
    <language>
        <name>remove task from tasks
reset expanded task</name>
        <nameStyle>42,7
</nameStyle>
    </language>
</elementInfo>
...
Ts0 f46 1493 277 22 22 14 0 #rect
...
' #txt
Ts0 f47 1486 180 36 24 20 -2 #rect
Ts0 f47 @|StepIcon #fIcon
Ts0 f49 expr out #txt
Ts0 f49 1504 107 1504 180 #arcP
Ts0 f50 expr out #txt
Ts0 f50 1504 204 1504 277 #arcP
...
Ts0 f56 1696 107 1696 277 #arcP
...
' #txt
Ts0 f60 actionCode 'import ch.ivy.addon.portalkit.service.FilterService;

FilterService filterService = new FilterService(in.keyword);
in.filteredTasks = filterService.filterTasks(in.tasks);
' #txt
...
' #txt
Ts0 f60 2174 180 36 24 20 -2 #rect
Ts0 f60 @|StepIcon #fIcon
Ts0 f61 expr out #txt
Ts0 f61 2192 107 2192 180 #arcP
...
' #txt
Ts0 f65 actionCode 'in.keyword = null;
in.filteredTasks = in.tasks;' #txt
...
' #txt
Ts0 f65 2054 180 36 24 20 -2 #rect
Ts0 f65 @|StepIcon #fIcon
Ts0 f66 expr out #txt
Ts0 f66 2072 107 2072 180 #arcP
Ts0 f64 expr out #txt
Ts0 f64 2072 204 2072 277 #arcP
Ts0 f44 expr out #txt
Ts0 f44 2192 204 2192 277 #arcP
Ts0 f67 guid 15307543DA9F127B #txt
Ts0 f67 type ch.ivy.addon.portalkit.component.TaskWidget.TaskWidgetData #txt
Ts0 f67 method setTasks(java.util.List<ch.ivy.addon.portalkit.bo.RemoteTask>) #txt
Ts0 f67 disableUIEvents false #txt
Ts0 f67 inParameterDecl 'ch.ivyteam.ivy.richdialog.exec.RdMethodCallEvent methodEvent = event as ch.ivyteam.ivy.richdialog.exec.RdMethodCallEvent;
<java.util.List<ch.ivy.addon.portalkit.bo.RemoteTask> tasks> param = methodEvent.getInputArguments();
' #txt
Ts0 f67 inParameterMapAction 'out.tasks=param.tasks;
' #txt
Ts0 f67 outParameterDecl '<> result;
' #txt
...
' #txt
Ts0 f67 1269 357 22 22 14 0 #rect
Ts0 f67 @|RichDialogMethodStartIcon #fIcon
Ts0 f68 type ch.ivy.addon.portalkit.component.TaskWidget.TaskWidgetData #txt
Ts0 f68 1269 509 22 22 14 0 #rect
Ts0 f68 @|RichDialogProcessEndIcon #fIcon
Ts0 f70 actionDecl 'ch.ivy.addon.portalkit.component.TaskWidget.TaskWidgetData out;
' #txt
Ts0 f70 actionTable 'out=in;
' #txt
Ts0 f70 actionCode 'import ch.ivy.addon.portalkit.service.FilterService;

if ("".equals(in.keyword)) {
	in.filteredTasks = new List();
	in.filteredTasks.addAll(in.tasks);
} else {
	FilterService filterService = new FilterService(in.keyword);
	in.filteredTasks = filterService.filterTasks(in.tasks);
}
...
' #txt
Ts0 f70 1262 428 36 24 20 -2 #rect
Ts0 f70 @|StepIcon #fIcon
Ts0 f71 expr out #txt
Ts0 f71 1280 379 1280 428 #arcP
Ts0 f69 expr out #txt
Ts0 f69 1280 452 1280 509 #arcP
Ts0 f72 guid 15336D4C94B1C3AD #txt
Ts0 f72 type ch.ivy.addon.portalkit.component.TaskWidget.TaskWidgetData #txt
Ts0 f72 method findPreExpandedTask(ch.ivy.addon.portalkit.bo.RemoteTask,Boolean) #txt
...
<ch.ivy.addon.portalkit.bo.RemoteTask task,java.lang.Boolean isPreExpandedTask> param = methodEvent.getInputArguments();
' #txt
...
Ts0 f45 mainOut f49 tail #connect
...
Ts0 f67 mainOut f71 tail #connect
Ts0 f71 head f70 mainIn #connect
Ts0 f70 mainOut f69 tail #connect
