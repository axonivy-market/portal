...
public final class ChatMessageManager {
  ...
  private static final String CONVERSATION_FILE_PATH = java.io.File.separator + "conversations" + java.io.File.separator + "%s.token";
    ...
    try {
      ...
      if (StringUtils.isNotEmpty(conversation)) {
        ...
        for (String record : records) {
          String decryptedRecord = SecureMessage.decrypt(record, fileName);
          Message message = new MessageDecoder().decode(decryptedRecord);
          messages.add(message);
        }
      }
    } catch (IOException e) {
      Ivy.log().warn("Could not load previous conversation.", e);
    }
    return messages;
  }
  ...
  public static void saveMessage(Message message) {
    ...
    String encryptedMessage = SecureMessage.encrypt(convertedMessage, fileName);
    try {
