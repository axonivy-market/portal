...
public final class TaskUtils {
   ...
   * 
   * @param startIndex
   *          : the index of the first task of all tasks found returned
   * @param pageSize
   *          : how many tasks found are returned. -1 for all.
   * @param sortField
   *          : Field to sort the task list
   * @param sortOrder
   *          : Order to sort, ascending or descending
   * @param isHistory
   *          : true => finished task , else : running task
   * @param taskFilter
   *          : filter criteria to specify which tasks should be read
   * @return List<ITask> : the list task of session user the task of session
   *         user by some Filter properties
   */
  public static List<ITask> findTaskOfSessionUser(int startIndex, int pageSize, String sortField, SortOrder sortOrder,
    ...
    List<PropertyOrder<TaskProperty>> taskPropertyOrder = PropertyOrder.create(getTaskProperty(sortField),
	getTaskDirection(sortOrder));
    if (isHistory) {
      queryResult = ivy.session.findWorkedOnTasks(taskFilter, taskPropertyOrder, startIndex, pageSize, true);
    } else {

      queryResult = ivy.session.findWorkTasks(taskFilter, taskPropertyOrder, startIndex, pageSize, true,
  ...
  /**
   * Park The Task. If it is SUSPENDED state, then change to RESUMED first, then
   * park it.
   * 
   * @param task
   *          : Task need to be park
   * @throws Exception
   *           exception
   */
  ...
  public static ITask findWorkOnTaskById(long taskId) {
    Ivy ivy = Ivy.getInstance();
    IPropertyFilter<TaskProperty> taskFilter = ivy.session.getWorkflowContext().createTaskPropertyFilter(
	TaskProperty.ID, RelationalOperator.EQUAL, taskId);
    IQueryResult<ITask> queryResult = ivy.session.findWorkTasks(taskFilter, null, 0, 1, true,
	EnumSet.of(TaskState.SUSPENDED, TaskState.RESUMED, TaskState.PARKED));
    List<ITask> tasks = queryResult.getResultList();
   ...
   * @param taskId
   *          task id to find
   * @return task if it exists and user has insufficient rights to see, null if
   *         otherwise
   */
  ...
  public static List<TaskVO> filterCurrentTasksByCase(ICase iCase) {
    ...
    if (iCase != null && iCase.getTasks() != null && iCase.getTasks().size() > 0) {
      EnumSet<TaskState> notDoneStates = EnumSet.of(TaskState.SUSPENDED, TaskState.RESUMED, TaskState.PARKED,
	  TaskState.DELAYED, TaskState.WAITING_FOR_INTERMEDIATE_EVENT);
   ...
   *
   * @param iCase
   *          : iCase value to get all the task
   * @param taskId
   *          : id value of task
   * @return ITask value
   */
   ...
   * 
   * @param iTask
   *          ITask object to get the email
   * @return String get the email of creator of case, execute under system
   *         permission
   */
   ...
   * @param iTask
   *          ITask object to get the email
   * @return String get phone number of creator of case, execute under system
   *         permission
   */
   ...
   * @param iTask
   *          ITask object to get the email
   * @return String get mobile number of creator of case, execute under system
   *         permission
   */
   ...
   * 
   * @return True : has read all tasks permission, False : do not have read all
   *         tasks permission
   */
  public static boolean checkReadAllTasksPermission() {
    boolean hasReadAllTasksPermission = Ivy
	.session()
	.getSecurityContext()
  ...
  public static boolean checkReadAllCasesPermission() {
    boolean hasReadAllCasesPermission = Ivy
	.session()
	.getSecurityContext()
	.hasPermission(Ivy.request().getApplication().getSecurityDescriptor(),
  ...
  public static List<ITask> findWaitingTaskByKindCode(final String kindCode) {
	...
	public List<ITask> call() throws Exception {
	  try {
	    TaskQuery taskQuery = TaskQuery.create().where().state().isEqual(TaskState.WAITING_FOR_INTERMEDIATE_EVENT)
		.and().kindCode().isEqual(kindCode);
  ...
  /**
   * Get all task in the system. Only for administrator who has read all tasks
   * and administrator workflow permission
   * 
  ...
  public static List<ITask> findAllTasks(final int startIndex, final int pageSize, final String sortField,
    ...
    List<PropertyOrder<TaskProperty>> taskPropertyOrder = PropertyOrder.create(
	ch.ivy.addon.portalkit.util.TaskUtils.getTaskProperty(sortField),
	ch.ivy.addon.portalkit.util.TaskUtils.getTaskDirection(sortOrder));
    if (isHistory) {
      queryResult = ivy.session.findWorkedOnTasks(taskFilter, taskPropertyOrder, startIndex, pageSize, true);
    } else {
      if (checkReadAllTasksPermission()) {
	IPropertyFilter<TaskProperty> filter = Ivy.wf().createTaskPropertyFilter(TaskProperty.STATE,
	    RelationalOperator.EQUAL, TaskState.SUSPENDED.intValue());
