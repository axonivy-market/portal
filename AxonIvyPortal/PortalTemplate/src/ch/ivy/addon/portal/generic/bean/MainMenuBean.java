...
public class MainMenuBean implements Serializable {
  ...
  private DefaultTreeNode buildTaskMenuItem(List<String> categories, boolean isAllTaskNode) {
    ...
    tasksMenuItem.setRootAllTask(isAllTaskNode);
  ...
  private DefaultTreeNode buildTaskTree(String nodeDisplayName, List<String> categories, String firstCategory) {
    ...
    boolean isRootAllTask = firstCategory.equals(TreeNodeType.TASKS_ALL_TASKS);
    taskMenuItem.setRootAllTask(isRootAllTask);
    DefaultTreeNode taskNode = new DefaultTreeNode(taskMenuItem);
    taskNode.setType(firstCategory);
    List<TreeNode> childrenNodes =
        TaskTreeUtils.convertTaskListToTree(categories, firstCategory, isRootAllTask).getChildren();
    taskNode.setChildren(childrenNodes);
    taskNode.setExpanded(true);
    return taskNode;
  }
  ...
  public TreeNode initMyTasksNode() {
    ...
    personalTaskNode.setType(hasReadAllTasksPermisson ? TreeNodeType.TASKS_MY_TASKS : TreeNodeType.TASKS);
    return personalTaskNode;
  }

  public List<RemoteCase> getCasesOfSelectedCategory(TreeNode selectedCategory, String keyword) {
    if (selectedCategory == null && !keyword.isEmpty()) {
      selectedCategory = initMyCasesNode();
    }
    if (selectedCategory == null && keyword.isEmpty()) {
      selectedCategory = initCaseNodeLinkedFromTask();
    }
  ...
  public TreeNode initMyCasesNode() {
    ...
    personalCaseNode.setType(hasReadAllCasesPermisson ? TreeNodeType.CASES_MY_CASES : TreeNodeType.CASES);
    return personalCaseNode;
  }

  public TreeNode initCaseNodeLinkedFromTask() {
    DefaultTreeNode caseNode = new DefaultTreeNode();
    caseNode.setType(hasReadAllCasesPermisson ? TreeNodeType.CASES_ALL_CASES : TreeNodeType.CASES);
    return caseNode;
  }
