...
public class UserMenuBean {
  ...
  private String findPortalHomeFullPath() {
    return findProcessByUserFriendlyRequestPath("Start Processes/PortalTemplatePages/PortalHome.ivp");
  }
    ...
    for (IApplication application : applicationConfigurationManager.getApplications()) {
      ...
      IProcessStart processStart =
          processStartCollector
              .findProcessStartByUserFriendlyRequestPath(userFriendlyRequestPath);
      if (processStart != null) {
        try {
          UrlDetector urlDetector = new UrlDetector();
          String baseUrl = urlDetector.getBaseURL(FacesContext.getCurrentInstance());
          return baseUrl + "/pro/" + processStart.getFullRequestPath();
