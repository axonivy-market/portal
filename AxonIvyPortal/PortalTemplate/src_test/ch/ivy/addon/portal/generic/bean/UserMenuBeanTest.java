...
public class UserMenuBeanTest {
  ...
  public void testGetHomePageURL() throws Exception {
    final String HOMEPAGE_URL = "HOMEPAGE_URL";

    GlobalSettingService globalSettingService = mock(GlobalSettingService.class);
    whenNew(GlobalSettingService.class).withNoArguments().thenReturn(globalSettingService);
    when(globalSettingService.findGlobalSettingValue(HOMEPAGE_URL)).thenReturn("HomeURL");
    assertEquals("HomeURL", userMenuBean.getHomePageURL());

    verifyNew(GlobalSettingService.class).withNoArguments();
    Mockito.verify(globalSettingService).findGlobalSettingValue(HOMEPAGE_URL);
  }
  ...
  public void testSearchInCaseNotEmptyKeyword() throws Exception {
    ...
    GlobalSearchEvent globalSearchEvent = new GlobalSearchEvent(keyWord, StringUtils.EMPTY);
