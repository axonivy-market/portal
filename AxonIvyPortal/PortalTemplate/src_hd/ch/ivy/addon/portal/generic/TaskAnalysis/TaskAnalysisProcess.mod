...
import ch.ivy.addon.portalkit.datamodel.TaskAnalysisLazyDataModel;
	...
	out.taskView = TaskAnalysisView.create().dataModel(dataModel).pageTitle(pageTitle).hideTaskFilter(false).showHeaderToolbar(false).noTaskFoundMessage(noTaskMessage).createNewTaskView();
' #txt
