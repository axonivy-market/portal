 ...
 *
 */
public class LanguagesSettingsServiceImpl extends AbstractService implements
ILanguagesSettingsService {
  
	private final String CMS_LANG_KEY = "/PortalKit/AppInfo/SupportedLanguages";
	
	@Override
	public LanguagesSettingsServiceResult getLanguagesSettings(String username,
			List<String> apps, Long serverId){
	...
	public IvyLanguageSetting getUserLanguagesSettings(final String username,final String appName, final Long serverId) throws Exception{
	...
	@Override
	public LanguagesSettingsServiceResult setLanguagesSettings(String username,
			List<IvyLanguageSetting> settings) {
	 ...
	 * @throws Exception
	 */
	private List<WSException> saveLanguagesSettings(final String appName, final String username, final IvyLanguageSetting setting) throws Exception{
		return ServerFactory.getServer().getSecurityManager().executeAsSystem(new Callable<List<WSException>>() {
			...
			public List<WSException> call() throws Exception {
				List<WSException> errors = new ArrayList<WSException>();
				IApplication application = ServerFactory.getServer().getApplicationConfigurationManager().findApplication(appName);
				...
				if(application != null){
					...
					if(user != null){
						...
						if (user.getEMailNotificationSettings().isUseApplicationDefault()){
							...
							userEmailSettings.setSendDailyTaskSummary(application.getDefaultEMailNotifcationSettings().getSendDailyTaskSummary());
							userEmailSettings.setNotificationDisabled(application.getDefaultEMailNotifcationSettings().isNotificationDisabled());
