...
public class TaskServiceImpl extends AbstractService implements ITaskService {
      ...
      return ServerFactory
          ...
          .executeAsSystem(
              () -> {
                ...
                for (ITask task : tasks) {
                  IvyTask ivyTask = transformer.transform(task);
                  ivyTask.setCanReset(SessionUtil.doesUserHavePermission(task.getApplication(),
                      taskSearchCriteria.getInvolvedUsername(), IPermission.TASK_RESET_OWN_WORKING_TASK));
                  ivyTask.setCanDelegate(SessionUtil.doesUserHavePermission(task.getApplication(),
                      taskSearchCriteria.getInvolvedUsername(), IPermission.TASK_WRITE_ACTIVATOR));
                  ivyTask.setCanResume(true);
                  ivyTasks.add(ivyTask);
                }
      ...
      return ServerFactory.getServer().getSecurityManager().executeAsSystem(new Callable<TaskServiceResult>() {
        ...
        public TaskServiceResult call() throws Exception {
          ...
          for (ITask task : tasks) {
            IvyTask ivyTask = transformer.transform(task);
            ivyTask.setServerId(serverId);
            IApplication application = task.getApplication();
            boolean canUserResumeTask = canUserResumeTask(username, task);
            ivyTask.setCanReset(hasPermissionToResetTask(application, username, canUserResumeTask));
            ivyTask.setCanDelegate(hasPermissionToDelegateTask(application, username));
  ...
  @Override
  public TaskServiceResult findTasksOfCase(final Long caseId, String serverUrl) throws WSException {
    try {
      TaskQuery query = TaskQuery.create().where().caseId().isEqual(caseId);
      TaskServiceResult result = findTasksByQuery(query, serverUrl);
      return result;
    } catch (Exception e) {
  ...
  private TaskServiceResult findTasksByQuery(TaskQuery query, String serverUrl) throws Exception {
    return ServerFactory.getServer().getSecurityManager().executeAsSystem(() -> {
      ...
      List<IvyTask> ivyTasks = new ArrayList<>();
      for (ITask task : tasks) {
        ivyTasks.add(transformer.transform(task));
      }
  ...
  private boolean hasPermissionToResetTask(IApplication application, String username, boolean canUserResumeTask) {
    return (SessionUtil.doesUserHavePermission(application, username, IPermission.TASK_RESET_OWN_WORKING_TASK) && canUserResumeTask)
        || SessionUtil.doesUserHavePermission(application, username, IPermission.TASK_RESET);
  }

  private boolean hasPermissionToDelegateTask(IApplication application, String username) {
    return SessionUtil.doesUserHavePermission(application, username, IPermission.TASK_WRITE_ACTIVATOR);
  }
  ...
  private boolean canUserResumeTask(String userName, ITask task) {
    ...
    boolean canUserResumeTask = task.getActivatorUserCandidates().contains(user);
