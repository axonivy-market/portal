...
class ClientChart {
  ...
  updateButtonAriaLabels(chart, widgetName, additionalConfig) {
    let expandTemplate, collapseTemplate, infoTemplate;
    if (additionalConfig) {
      additionalConfig.forEach(function (item) {
        if (item && item.expandLabelTemplate) expandTemplate = item.expandLabelTemplate;
        if (item && item.collapseLabelTemplate) collapseTemplate = item.collapseLabelTemplate;
        if (item && item.infoLabelTemplate) infoTemplate = item.infoLabelTemplate;
      });
    }
    let cardPanel = $(chart).parents('.card-widget-panel');
    if (expandTemplate) {
      let btn = cardPanel.find('.expand-link').get(0);
      if (btn) btn.setAttribute('aria-label', expandTemplate.replace('{0}', widgetName));
    }
    if (collapseTemplate) {
      let btn = cardPanel.find('.collapse-link').get(0);
      if (btn) btn.setAttribute('aria-label', collapseTemplate.replace('{0}', widgetName));
    }
    if (infoTemplate) {
      let btn = cardPanel.find('.widget__info-sidebar-link').get(0);
      if (btn) btn.setAttribute('aria-label', infoTemplate.replace('{0}', widgetName));
    }
  }
}
...
class ClientCanvasChart extends ClientChart {
  ...
  initWidgetTitle() {
    ...
    this.updateButtonAriaLabels(this.chart, this.data.chartConfig.name, this.data.chartConfig.additionalConfig);
  }
...
class ClientNumberChart extends ClientChart {
  ...
  render() {
    let config = this.data.chartConfig;
    this.initWidgetHeaderName(this.chart, config.name, config.additionalConfig);
  ...
  initWidgetHeaderName(chart, widgetName, additionalConfig) {
    ...
    this.updateButtonAriaLabels(chart, widgetName, additionalConfig);
  }
