...
public class TaskDetailsBean extends AbstractConfigurableContentBean<TaskDetails> implements Serializable {
  ...
  private String parkedInfoBannerMessage(ITask selectedTask) {
    ...
    return Ivy.cms().co(TASK_PARKED_BY_OTHER_INFO_CMS_URI, Arrays.asList(getWorkerUser(selectedTask)));
  }
  ...
  public String getDestroyTaskMessage() {
    ...
    if (task != null) {
      return Ivy.cms().co("/ch.ivy.addon.portalkit.ui.jsf/taskList/destroyTaskMessage", List.of(task.names().current(), Long.toString(task.getId())));
    }
