...
public class PortalCustomFieldUtils {
  ...
  private static boolean hasCmsValues(String columnField, Set<ICustomFieldMeta> customFieldMetaList) {
    for (ICustomFieldMeta customFieldMeta : customFieldMetaList) {
      if (customFieldMeta.name().equals(columnField)
          && Boolean.valueOf(customFieldMeta.attribute(HAS_CMS_VALUES_ATTRIBUTE))) {
        return true;
      }
    }
    return false;
  ...
  public static List<String> getCmsValuesMatchingWithKeywordList(String columnField, DashboardColumnType type,
    ...
    icustomFieldMetaList.stream()
        .filter(customField -> customField.type() == CustomFieldType.STRING && customField.name().equals(columnField))
        .forEach(field -> {
  ...
  public static List<String> getAllLocalizedValueOnCaseField(String columnField) {
    ICustomFieldMeta meta =
        ICustomFieldMeta.cases().stream().filter(m -> columnField.equals(m.name())).findAny().orElse(null);
    return getAllLocalizedValueByField(meta);
  }

  public static List<String> getAllLocalizedValueOnTaskField(String columnField) {
    ICustomFieldMeta meta =
        ICustomFieldMeta.tasks().stream().filter(m -> columnField.equals(m.name())).findAny().orElse(null);
    return getAllLocalizedValueByField(meta);
  }
