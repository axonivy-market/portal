...
public class CustomTextIsEmptyOperatorHandler {
  ...
  public CaseQuery buildIsEmptyQuery(DashboardFilter filter) {
    ...
    subQuery.where().customField().textField(filter.getField()).isNull().or().customField().textField(filter.getField()).isLike("");
    return subQuery;
  }

  public CaseQuery buildNotEmptyQuery(DashboardFilter filter) {
    CaseQuery subQuery = initCaseQuery(filter.getFilterType());
    subQuery.where().customField().textField(filter.getField()).isNotNull().and().customField().textField(filter.getField()).isNotLike("");
    return subQuery;
  }
}
