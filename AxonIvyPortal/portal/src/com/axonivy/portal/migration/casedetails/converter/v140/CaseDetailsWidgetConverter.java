...
/**
 * Brings a Case Details configuration written before 14.0.0 up to the redesigned layout. Its
 * widgets are {@code summary}, {@code information}, {@code document}, {@code relatedTask},
 * {@code technicalCase}, {@code history}, {@code businessDetails} and {@code custom}. Two things
 * changed:
 *
 * <ol>
 * <li><b>Grid unit.</b> GridStack {@code cellHeight} went from 100px to 20px in
 * {@code case-details.js} while {@code column} stayed at 12 — {@code gridstack.min.css} bakes in
 * the 1/12 geometry, so it could not be raised. Therefore {@code y} and {@code h} are multiplied
 * by {@value #CELL_HEIGHT_RATIO} and {@code x}/{@code w} are left alone:
 ...
 * (case id / state / category) and the field list in one card; those are now two widgets,
 ...
 * total footprint is unchanged and no other widget has to move. That is exactly how the shipped
 * default was authored: the old {@code information h6} became {@code summary h12} +
 * {@code information h18} (12 + 18 = 6 * 5).</li>
 * </ol>
 */
public class CaseDetailsWidgetConverter implements IJsonConverter {
  ...
  public AbstractJsonVersion version() {
    return new CaseDetailsJsonVersion("14.0.0");
  }
