package ch.ivy.addon.portalkit.test.bean;
...
//Copied from PortalKit, no changes
public class PortalPermissionInitBean extends AbstractProcessStartEventBean {

  public PortalPermissionInitBean() {
    super("Init Portal Permissions for cluster test", "Create Portal permissions if missing");
  }
  ...
  private IPermissionGroup createPortalPermissionGroup() {
    IPermissionGroup rootGroup =
        IApplication.current().getSecurityDescriptor().getSecurityDescriptorType().getRootPermissionGroup();
