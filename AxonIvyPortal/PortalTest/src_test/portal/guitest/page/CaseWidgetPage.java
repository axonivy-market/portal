...
public class CaseWidgetPage extends TemplatePage {
  ...
  public String getCaseName() {
    ...
    WebElement selectedCaseNameElement = findElementById(selectedCaseElement.getAttribute("id") + ":case-name-component:case-name-form:case-name-edit-inplace");
  ...
  public void openAdvancedFilter(String filterName, String filterIdName) {
    ...
    waitForElementDisplayed(By.cssSelector("span[id$='" + filterIdName + "-filter:filter-open-form:advanced-filter-item-container']"), true);
  }
  ...
  public String getFilterValue(String filterId) {
    Awaitility.await().atMost(new Duration(5, TimeUnit.SECONDS)).until(() -> findElementByCssSelector("button[id$='" + filterId + ":filter-open-form:advanced-filter-command']").getText().length() > 1);
  ...
  public void waitUntilCaseCountDifferentThanZero() {
    Awaitility.await().atMost(new Duration(5, TimeUnit.SECONDS)).until(() -> getCaseCount().intValue() != 0);
  }
  ...
  public void clickColumnCheckboxByName(String columnName) {
    ...
    findListElementsByCssSelector("label[for*=':case-columns-configuration:select-columns-form:columns-checkbox:']").stream().filter(checkbox -> checkbox.getText().equalsIgnoreCase(columnName)).findAny()
  ...
  public boolean isUserDisplayInCreatorFilter(String userFullName) {
    ...
    waitAjaxIndicatorDisappear();
    try {
      Awaitility.await().atMost(new Duration(5, TimeUnit.SECONDS)).until(() -> findElementByCssSelector("span[id$='filter-input-form:creator-component:creator-select_panel']").isDisplayed());
