...
public class DocumentUploadBean implements Serializable {
  ...
  public boolean getPortalScriptCheckingSettingOrDefault(boolean defaultIfEmpty) {
    return GlobalSettingService.getInstance().findGlobalSettingValueAsBoolean(GlobalVariable.ENABLE_SCRIPT_CHECKING_FOR_UPLOADED_DOCUMENT.toString(), defaultIfEmpty);
  }

  public boolean getPortalVirusScannerSettingOrDefault(boolean defaultIfEmpty) {
    return GlobalSettingService.getInstance().findGlobalSettingValueAsBoolean(GlobalVariable.ENABLE_VIRUS_SCANNER_FOR_UPLOADED_DOCUMENT.toString(), defaultIfEmpty);
  }

  public String getPortalAllowedUploadFileTypesSettingOrDefault(String defaultIfEmpty) {
    return GlobalSettingService.getInstance().findGlobalSettingValueAsString(GlobalVariable.UPLOAD_DOCUMENT_WHITELIST_EXTENSION.toString(), defaultIfEmpty);
  }
}
