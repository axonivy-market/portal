...
public class CaseDocumentService {
	...
	/**
	 * We make it as deprecated from 8.0 Please refer to sub-function process
	 * DeleteDocument
	 * 
	 * @param document
	 */
  ...
  public NewFilenameValidation isNewFilenameValid(IvyDocument modifiedDoc) {
	...
	private IDocumentService documentsOf(ICase iCase) {
		try {
			return ISecurityManager.instance().executeAsSystem(() -> iCase.documents());
		} catch (Exception e) {
	...
	private List<IDocument> getAllDocumentsOf(ICase iCase) {
		try {
			return ISecurityManager.instance().executeAsSystem(() -> iCase.documents().getAll());
		} catch (Exception e) {
			throw new PortalException(e);
		}
	}
