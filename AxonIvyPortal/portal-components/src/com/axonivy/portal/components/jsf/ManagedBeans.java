...
public class ManagedBeans {
	...
	/**
	 * Returns the instance of the specified {@code beanName} from the current
	 * {@code FacesContext}. If the bean cannot be obtained,
	 * {@code RuntimeException} will be thrown.
	 * 
	 * @param beanName
	 * 
	 * @return instance of the bean associating with {@code beanName}. Never returns
	 *         {@code null}.
	 * @throws NoSuchElementException if the bean is not present at the time of this
	 *                                invocation
	 * @see #find(String)
	 * 
	 */
	public static <T> T get(final String beanName) {
		...
		return beanInstance
				.orElseThrow(() -> new NoSuchElementException("The bean " + beanName + " does not exist yet"));
	}

	/**
	 * Returns an {@code Optional} instance of the given {@code beanName}. Unlike
	 * {@linkplain #get}, this method will not throw exception if the bean is not
	 * present. However, the client of this method should check for present of the
	 * bean via the returned {@code Optional}, otherwise, you'll get
	...
	public static <T> Optional<T> find(final String beanName) {
		try {
			return ISecurityManager.instance().executeAsSystem(() -> {
				FacesContext context = FacesContext.getCurrentInstance();

				@SuppressWarnings("unchecked")
				T beanInstance = (T) context.getApplication().evaluateExpressionGet(context, "#{" + beanName + "}",
						Object.class);
