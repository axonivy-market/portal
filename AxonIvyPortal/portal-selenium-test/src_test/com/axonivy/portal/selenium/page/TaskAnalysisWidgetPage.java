...
public class TaskAnalysisWidgetPage extends TemplatePage {
  ...
  public void filterByCaseState(List<String> selectedPriorities) {
    ...
    var elmList = selectionElement.$$(By.tagName("LABEL")).texts();
    for (String label : elmList) {
      var labelElement = findElementByCssSelector("div[id$='task-widget:case-filters-container'] div[id$='state-filter:filter-input-form:advanced-filter-panel-content']")
          .$$("label").filter(Condition.text(label)).first();
