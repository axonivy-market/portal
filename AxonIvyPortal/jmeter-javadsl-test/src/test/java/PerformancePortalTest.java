...
public class PerformancePortalTest {
  ...
  TestPlanStats stats = testPlan(
      threadGroup("10 Users performance test")
        .rampTo(1,      // portal.thread.numberOfUser
          Duration.ofSeconds(1))   // portal.thread.rampup
      ...
      // Remove comment on local to debug, since server doesn't have the UI
      resultsTreeVisualizer(),
      htmlReporter("target/html-report")
    ).runIn(new EmbeddedJmeterEngine().propertiesFile("src/test/resources/test.properties"));
  }
