...
public class TaskWidgetPage extends TemplatePage {
  ...
  private void clickOnTaskEntryInFullMode(int index, boolean isDetailsShown) {
    WebElement taskShowHideDetailsLink =
        findElementByCssSelector("*[id$='" + index + ":task-item:task-info']");
  ...
  public TaskDetailsPage getTaskDetailsElement(int index) {
    waitForElementDisplayed(By.cssSelector("*[id$='" + index + ":task-item:task-info']"), true,
        DEFAULT_TIMEOUT);
  ...
  public long getIdOfTaskHasIndex(int index) {
    String text = findElementById("task-widget:task-list-scroller:"+ index + ":task-item:task-start-item-view:task-start-task-state").getText().replaceAll("\\D", "");
