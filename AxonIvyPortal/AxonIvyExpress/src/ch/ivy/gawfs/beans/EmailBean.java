...
public class EmailBean implements Serializable {
  ...
  public void uploadAttachment(FileUploadEvent event) throws IOException {
    ...
    if (!StringUtils.isEmpty(uploadDocumentCheckMessage)) {
      FacesContext.getCurrentInstance().addMessage(null, FacesMessageUtils.sanitizedMessage(FacesMessage.SEVERITY_ERROR, uploadDocumentCheckMessage, null));
      return;
    }

    List<ExpressAttachment> attachments = userEmail.getAttachments();
    boolean isDuplicatedName = attachments.stream().anyMatch(attachment -> (attachment.getStatus() != ExpressEmailAttachmentStatus.DELETED && uploadedFile.getFileName().equals(attachment.getName())));
    if (isDuplicatedName) {
  ...
  public void downloadAttachment(ExpressAttachment attachment) throws IOException {
    if (attachment.getContent() != null) {
      file = new DefaultStreamedContent(new ByteArrayInputStream(attachment.getContent()), attachment.getContentType(), attachment.getName());
      ...
      file = new DefaultStreamedContent(is, attachment.getContentType(), attachment.getName());
    }
  }
