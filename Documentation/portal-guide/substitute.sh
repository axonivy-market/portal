...
find source -name "*.rst" -exec grep -IlE "\\|(ivy|version|major)\\|" {} \; | while read -r rst; do
...
if [ -d "${PO_BASE}" ]; then
  ...
  find "${PO_BASE}" -name "*.po" | while read -r po; do
    ...
    if grep -qE "\\|(ivy|version|major)" "$po"; then
      # Use perl for multi-line matching and replacement
      perl -i -0pe '
        s/\|ivy-designer("\s*")\|/Axon Ivy Designer$1/g;
        s/\|ivy-designer\|/Axon Ivy Designer/g;
        s/\|ivy-engine("\s*")\|/Axon Ivy Engine$1/g;
        s/\|ivy-engine\|/Axon Ivy Engine/g;
        s/\|ivy("\s*")\|/Axon Ivy$1/g;
        s/\|ivy\|/Axon Ivy/g;
        s/\|version("\s*")\|/'"${version}"'$1/g;
        s/\|version\|/'"${version}"'/g;
        s/\|major-version("\s*")\|/'"${major}"'$1/g;
        s/\|major-version\|/'"${major}"'/g;
      ' "$po"
    fi
  done
