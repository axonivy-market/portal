...
deployment </doc/10.0.2/engine-guide/deployment/index.html>`
.
...
.. important::
   ...
   </doc/10.0.2/axonivy/migration/index.html>`. Changes in Axon Ivy could lead
...
#. Upgrade your projects to use the latest portal version by using the **Convert Project** feature. Please find more information here: :dev-url:`Converting Projects </doc/10.0.2/designer-guide/process-modeling/projects/converting.html#converting-projects>`
...
#. The ``customization.css`` file has been removed, in case you use it in your project, please switch to using
   :dev-url:`Engine Branding </doc/10.0.2/designer-guide/user-interface/branding/branding-engine.html>` to customize styling
...
#. Portal dashboard widgets only support the ``CustomFields`` declared in the ``custom-fields.yaml`` file.
   If your ``CustomFields`` are used in the dashboard widget, please follow the :dev-url:`Custom Fields Meta Information </doc/10.0.2/designer-guide/how-to/workflow/custom-fields.html#meta-information>` to adapt the data.
...
#. The ``DefaultChartColor.p.json`` sub process has been removed, in case you use it in your project, please remove override this sub process and switch to using
   :dev-url:`Engine Branding </doc/10.0.2/designer-guide/user-interface/branding/branding-engine.html>` to customize chart, data labels, legend color.
...
#. The callable process ``DefaultChart.p.json``, ``DefaultUserProcess.p.json`` has been removed. They are replaced by
   the :dev-url:`Variables </doc/10.0.2/designer-guide/configuration/variables.html>` configuration approach,
...
#. Deprecated callable processes: ``OpenPortalSearch.mod``, ``OpenPortalTasks.mod``, ``OpenPortalTaskDetails.mod``, ``OpenPortalCases.mod``, ``OpenPortalCaseDetails.mod`` process.
   ...
   Portal recommends using :dev-url:`Axon Ivy HtmlOverride wizard </doc/10.0.2/designer-guide/how-to/overrides.html?#override-new-wizard>` to customize ``Portal HTML Dialog``
...
#. HOMEPAGE_URL (single Portal app mode) and registered application link (multi
   ...
   :dev-url:`Default Pages </doc/10.0.2/designer-guide/user-interface/default-pages/index.html>`
