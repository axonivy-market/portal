...
.. _statistic-sample-kpi-procurement-overview:
...
The sample charts aggregate on the custom fields declared in the ``custom-fields.yaml`` file of the
``portal-user-examples`` project, for example the :guilabel:`Group by` field ``Department`` and the :guilabel:`KPI`
field ``SpendAmount``:
...
``HasCmsValues: true`` lets the chart display the values of a custom field in the language of the user.
The sample cases store the values ``DepA`` to ``DepD`` in the ``Department`` field, and the chart displays
the labels defined in the CMS under ``/CustomFields/Cases/Department/Values/{value}``, for example
``Dep A`` in English and ``Abt A`` in German. Without this attribute, the chart displays the stored
value ``DepA``.

Refer to :doc-url:`Custom Fields </reference/configuration/custom-fields.html>`
for more information about declaring custom fields, and to :doc-url:`Localize Label, Description, Category
and Values </reference/configuration/custom-fields.html#localize-label-description-category-and-values>`
for more information about the CMS values.
