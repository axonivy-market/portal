...
ポータルでは、レガシー統計チャートウィジェットが非推奨になりました。代わりに新しい標準的な統計チャートが多数用意されています。詳細については、 :ref:`statistic-chart` のセクションを参照してください。
...
#. 入力ラベルをクリックして、タイトルを翻訳する。|translate-icon| アイコンをクリックして、コンテンツを翻訳します。翻訳に問題が生じた場合は、:ref:`enable-translation` を参照してください。
...
.. |news-feed-widget-overlay-panel| image:: ../../screenshots/new-dashboard/news-feed-widget-overlay-panel.png
   :alt: ダッシュボードの外部ページウィジェットのオーバーレイダイアログ
