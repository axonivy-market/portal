[Ivy]
[>Created: Mon May 23 10:22:12 ICT 2016]
...
' #txt
Cs0 f36 actionCode 'import ch.ivy.addon.portalkit.datamodel.CaseLazyDataModel;
import ch.ivy.addon.portal.generic.common.TreeNodeType;

String title = "";
CaseLazyDataModel dataModel = new CaseLazyDataModel();
if(in.selectedNode.type.equals(TreeNodeType.CASES)) {
	title = ivy.cms.co("/ch.ivy.addon.portalkit.ui.jsf/caseList/cases");
	dataModel.setIgnoreInvolvedUser(in.hasReadAllCasesPermission);
} else if(in.selectedNode.type.equals(TreeNodeType.CASES_MY_CASES)){
	title = ivy.cms.co("/ch.ivy.addon.portal.generic/CustomLinkGenerator/myCases");
	dataModel.setIgnoreInvolvedUser(false);
} else if(in.selectedNode.type.equals(TreeNodeType.CASES_ALL_CASES)){
	title = ivy.cms.co("/ch.ivy.addon.portal.generic/CustomLinkGenerator/allCases");
	dataModel.setIgnoreInvolvedUser(true);
}

out.caseView = ch.ivy.addon.portal.generic.view.CaseView.create().dataModel(dataModel).withTitle(title).buildNewView();' #txt
...
' #txt
Cs0 f13 actionCode 'import ch.ivy.addon.portal.generic.view.TaskView;
